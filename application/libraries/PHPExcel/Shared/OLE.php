...
class PHPExcel_Shared_OLE
{
    ...
    public static function LocalDate2OLE($date = null)
    {
        ...
        for ($i = 0; $i < 4; ++$i) {
            $hex = (int)($low_part % 0x100); // Konversi hasil pembagian ke integer
            $res .= pack('c', $hex);
            $low_part = (int)($low_part / 0x100); // Konversi hasil pembagian ke integer
        }
        for ($i = 0; $i < 4; ++$i) {
            $hex = (int)($high_part % 0x100); // Konversi hasil pembagian ke integer
            $res .= pack('c', $hex);
            $high_part = (int)($high_part / 0x100); // Konversi hasil pembagian ke integer
        }
