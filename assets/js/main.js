...
// Initialize the testimonial navigation slider
...
function updateVideoInfo(slideIndex) {
  ...
  const title = activeSlide.attr('data-title') || 'Default Title';
  const description = activeSlide.attr('data-description') || 'Default Description';
  
  $('#video-title-youtube').text(title);
  $('#video-description').text(description);
}

// Set initial video info on page load
updateVideoInfo(0);

// Update video info on slide change
$('.video-items-active').on('afterChange', function(event, slick, currentSlide) {
  updateVideoInfo(currentSlide);
});
